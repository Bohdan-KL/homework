...
def play_game(word_list):
    ...
    # play_games_count is how many hand user want play. We play while play_games_count is not zero.
    while play_games_count != 0:
        hand = deal_hand(HAND_SIZE)
        display_hand(hand)
        print()
        # if user don't use substitute before.
        if not is_substitute:
            hand, is_substitute = ask_substitute(hand)
            print()
        score_first = play_hand(hand, word_list)
        # if user don't replay before, ask user.
        if not is_replay:
